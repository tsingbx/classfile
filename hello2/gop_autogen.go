// Code generated by gop (Go+); DO NOT EDIT.

package main

import "github.com/goplus/yap"

const _ = true

type hello struct {
	yap.App
}
//line hello2/hello_yap.gox:1
func (this *hello) MainEntry() {
//line hello2/hello_yap.gox:1:1
	this.Get("/p/:id", func(ctx *yap.Context) {
//line hello2/hello_yap.gox:2:1
		ctx.Json__1(map[string]string{"id": ctx.Param("id")})
	})
//line hello2/hello_yap.gox:7:1
	this.Handle("/", func(ctx *yap.Context) {
//line hello2/hello_yap.gox:8:1
		ctx.Html__1(`<html><body>Hello, <a href="/p/123">Yap</a>!</body></html>`)
	})
//line hello2/hello_yap.gox:11:1
	this.Run(":8000")
}
func main() {
//line hello2/hello_yap.gox:11:1
	yap.Gopt_App_Main(new(hello))
}
